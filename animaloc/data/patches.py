__copyright__ = \
    ...
    Last modification: May 2025 by Daniel Chacon
    """
...
__all__ = ['save_batch_images', 'save_batch_images_with_indices','ImageToPatches', 'AnnotatedImageToPatches',
    'PatchesBuffer']
...
def save_batch_images(
    ...
    base_wo_extension, extension = os.path.splitext(basename)
...
def save_batch_images_with_indices(
    batch: torch.Tensor,
    basename: str,
    dest_folder: str,
    indices: list
    ...
    Args:
        ...
        basename (str) : parent image name, with extension
        dest_folder (str): destination folder path
        indices (list): list of indices corresponding to the batch images
    '''
    ...
    extension = extension.lstrip('.')  # Remove the leading dot from the extension
    for i, b in enumerate(range(batch.shape[0])):
        full_path = '_'.join([base_wo_extension, str(indices[i]) + '.']) + extension
        save_path = os.path.join(dest_folder, full_path)
        save_image(batch[b], fp=save_path)
    ...
    def get_annotated_limits(self, min_visibility: float = 0.1) -> Annotations:
        ...
        for idx , limit in self.get_limits().items():
            ...
            for anno_dict in self.annotations:
                ...
                if hasattr(old_anno, 'x_min') and hasattr(old_anno, 'y_min') \
                    and hasattr(old_anno, 'x_max') and hasattr(old_anno, 'y_max'):
                    ### Check if the center of the annotation is in the limits ###
                    x_center = int(old_anno.x_min + (old_anno.x_max - old_anno.x_min) / 2)
                    y_center = int(old_anno.y_min + (old_anno.y_max - old_anno.y_min) / 2)
                    
                elif hasattr(old_anno, 'x') and hasattr(old_anno, 'y'):
                    ### Check if the annotation is in the limits ###
                    x_center = int(old_anno.x)
                    y_center = int(old_anno.y)

                else:
                    raise ValueError(
                        'Annotation object must have x_min, y_min, x_max, y_max ' \
                            'or x and y attributes')

                if x_center <= limit.x_min or x_center >= limit.x_max \
                    or y_center <= limit.y_min or y_center >= limit.y_max:
                    continue
                ...
                if frac_area > min_visibility:
                    ...
                    img_name, img_ext = os.path.splitext(anno_dict['images'])
                    img_ext = img_ext.lstrip('.')  # Remove the leading dot from the extension
...
class PatchesBuffer:
    ...
    def _create_buffer(self) -> pandas.DataFrame:
        ...
        for img_name in tqdm(numpy.unique(self.data.images), desc='Creating the buffer'):
            # If self.root_dir is a PIL image, do not use it as a path
            if isinstance(self.root_dir, PIL.Image.Image):
                pil_image = self.root_dir
            else:
                # Otherwise, construct the path from root_dir and img_name
                img_path = os.path.join(self.root_dir, img_name)
                try:
                    pil_image = PIL.Image.open(img_path).convert('RGB')
                except FileNotFoundError:
                    raise FileNotFoundError(
                        f"Image {img_path} not found. Please check the path and try again.")
