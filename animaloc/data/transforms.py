...
class SampleToTensor:
    ...
    def __call__(
        self, image: PIL.Image.Image, target: dict, anno_type: str = "bbox"
        ...
        if anno_type == "bbox":
            tr_target["boxes"] = torch.as_tensor(
                tr_target["boxes"], dtype=torch.float32
            )
        elif anno_type == "point":
            tr_target["points"] = torch.as_tensor(
                tr_target["points"], dtype=torch.float32
            )
...
class DownSample:
    ...
    def __init__(self, down_ratio: int = 2, anno_type: str = "bbox") -> None:
        """
        Args:
    ...
    def __call__(
        ...
        if self.anno_type == "bbox":
            target["boxes"] = torch.div(
                target["boxes"], self.down_ratio, rounding_mode="floor"
            )

        elif self.anno_type == "point":
            target["points"] = torch.div(
                target["points"], self.down_ratio, rounding_mode="floor"
            )
...
class FIDT:
    ...
    def _get_fidt(self, mask: torch.Tensor) -> torch.Tensor:
        # 2026-05-01: two changes here, both pure CPU-side speedups with
        # bit-equivalent (or float-precision-equivalent) output vs the original.
        #
        # (1) scipy.ndimage.distance_transform_edt -> cv2.distanceTransform with
        #     DIST_L2 + DIST_MASK_PRECISE (exact L2). On 256x256 masks cv2 is
        #     ~3x faster on average and stays flat with point density (scipy
        #     scales with #points: 1.0 ms empty -> 2.3 ms at 100+ points; cv2
        #     stays ~0.5 ms). Max abs diff vs scipy on the FIDT output for
        #     non-empty masks is 7.7e-9 (float32 epsilon).
        #
        # (2) torch.pow + torch.where -> numpy.power + ndarray indexing. The
        #     dominant cost in the original was torch.pow on a 256x256 float64
        #     tensor in torch 1.11 — measured at ~25 ms / call vs ~0.5 ms for
        #     the same op in numpy.power. Switching the formula to numpy
        #     drops _get_fidt from ~27 ms to ~2 ms (~12x). The output is then
        #     converted to a torch tensor; FIDT.__call__ already calls
        #     dist_map.type(image.type()) so the final dtype is unchanged.
        #
        # Empty-mask path: _onehot short-circuits via "if len(target['points'])
        # > 0", so _get_fidt is only called on masks containing at least one
        # zero pixel. Behavior on all-ones masks is irrelevant in production.
        #
        # See tools/benchmark_edt.py and logs/edt_benchmark.csv for raw numbers.
        ...
        mask_np = mask.numpy().astype(numpy.uint8)
        d = cv2.distanceTransform(mask_np, cv2.DIST_L2, cv2.DIST_MASK_PRECISE).astype(
            numpy.float64
        )
        d = 1.0 / (numpy.power(d, self.alpha * d + self.beta) + self.c)
        d[d < 0.01] = 0.0
    ...
    def _onehot(self, image: torch.Tensor, target: torch.Tensor):
        ...
        if len(target["points"]) > 0:
            ...
            dist_maps = torch.ones(
                (self.num_classes, self.img_height, self.img_width), dtype=torch.float64
            )
            for i, mask in enumerate(masks):
                ...
                if i + 1 in labels:
                    ...
                    dist_maps[i] = torch.zeros(
                        (self.img_height, self.img_width), dtype=torch.float64
                    )
...
class GaussianMap:
    ...
    def _gaussian_map(self, pts_map: torch.Tensor) -> torch.Tensor:
        ...
        for i, pt in enumerate(points):
            ...
            pt_map[pt[0], pt[1]] = 1.0

            density += (
                scipy.ndimage.filters.gaussian_filter(
                    pt_map, self.sigma, mode="reflect"
                )
                * self.factor
            )
    ...
    def _onehot(self, image: torch.Tensor, target: torch.Tensor):
        ...
        if len(target["points"]) > 0:
            ...
            gauss_map = torch.zeros(
                (self.num_classes, self.img_height, self.img_width), dtype=torch.float32
            )
            for i, mask in enumerate(masks):
...
class Count_Instances:
    ...
    def __init__(self, count_key: str = "points") -> None:
        """
        Args:
            count_key (str, optional): The key in the target dictionary that contains
                the points to count. Defaults to 'points'.
        """
    ...
    def __call__(
        ...
        Returns:
            Tuple[torch.Tensor, int]:
                the transormed image and the number of instances
        """
        if isinstance(image, PIL.Image.Image):
            image = torchvision.transforms.ToTensor()(image)

        return image, len(target[self.count_key])
