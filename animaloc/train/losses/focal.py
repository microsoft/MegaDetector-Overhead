...
class FocalLoss(torch.nn.Module):
    ...
    def _neg_loss(self, output: torch.Tensor, target: torch.Tensor):
        ...
        pos_inds = target.eq(1).float() # mask (B,1,256,256) -> it is going to be True at that pixel x,y 
        neg_inds = target.lt(1).float() # mask (B,1,256,256) -> it is going to be False elsewhere

        neg_weights = torch.pow(1 - target, self.beta)

        loss = torch.zeros((B,C)) # (B, 1) and we have a detection at (x,y)
