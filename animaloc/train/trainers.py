...
class Trainer:
    ...
    def start(
        ...
        # Write end-of-training summary to validation log
        summary_msg = (
            f'Training complete | '
            f'Best {validate_on}: {self.best_val:.6f} at epoch {self.best_epoch}'
        )
        print(summary_msg)
        if self.val_logger.logger is not None:
            self.val_logger.logger.info(summary_msg)
...
from .two_stage_trainer import TwoStageFineTuneTrainer

# ---- Finalize what this module exports
__all__ += ['TRAINERS', 'Trainer']
if 'TwoStageFineTuneTrainer' in globals():
    __all__.append('TwoStageFineTuneTrainer')
# If you also have FasterRCNNTrainer defined above:
# __all__.append('FasterRCNNTrainer')

# Optionally also expose whatever the registry added
__all__ += list(TRAINERS.registry_names)