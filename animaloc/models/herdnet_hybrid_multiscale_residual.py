...
def _build_shifted_window_mask(Hp, Wp, window_size, shift_size, device, dtype):
    """
    Máscara de atención para SW-MSA siguiendo Swin original.
    Retorna (nW, ws*ws, ws*ws) con 0 o -inf.
    """
    if shift_size == 0:
        return None
...
class DropPath(nn.Module):
    """Drop paths por muestra (stochastic depth) sobre ramas residuales."""
...
# ---------------------------
# Swin: Atención por ventana
# ---------------------------
class WindowAttention(nn.Module):
    """ W-MSA con sesgo relativo. """
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        # tabla de sesgo relativo
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))

        # índices de posición relativa
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix: Optional[torch.Tensor] = None):
        """
        x: (B, H, W, C)
        mask_matrix: opcional; si None y shift>0, se genera internamente
        """
        B, H, W, C = x.shape
        shortcut = x
        x = self.norm1(x)

        # pad a múltiplos de window_size (sobre W y H en (B,H,W,C))
        pad_l = pad_t = 0
...
class MultiscaleSwinTransformer(nn.Module):
    """
    Swin multi-escala con residual por escala.
    - channels_list: lista de C por nivel (p.ej., [16, 32, 64, 128, 256, 512])
    - window_sizes: lista ws por nivel (default: [8,8,8,4,4,4])
    - num_heads_list: lista heads por nivel (default: [1,1,2,4,8,16])
    - num_layers_per_scale: int (replicado) o lista por nivel (default: [0,1,2,2,2,3])
    """
    def __init__(
        self,
        ...
        assert len(channels_list) == len(window_sizes) == len(num_heads_list) == len(num_layers_per_scale), \
            "channels_list, window_sizes, num_heads_list y num_layers_per_scale deben tener misma longitud"

        def _fit_heads(channels, prefer_heads):
            # Ajusta heads para C % heads == 0
        ...
        for C, ws, h, nl in zip(channels_list, window_sizes, num_heads_list, num_layers_per_scale):
            ...
            # construir nl bloques alternando shift 0 / ws//2
    ...
    def forward(self, feature_list: List[torch.Tensor]):
        """
        feature_list: lista de tensores (B, C, H, W)
        return: lista de tensores mejorados (B, C, H, W) con residual por escala
        """
        enhanced_features = []
        for features, scale_blocks in zip(feature_list, self.swin_blocks):
            if len(scale_blocks) == 0:
                # sin Swin en esta escala → passthrough
...
class HerdNet_Hybrid_Multiscale_Residual(nn.Module):
    ...
    def __init__(
        ...
        assert down_ratio in [1, 2, 4, 8, 16], f"down_ratio debe ser 1/2/4/8/16, got {down_ratio}"
        base_name = f'dla{num_layers}'

        self.down_ratio = down_ratio
        self.head_conv = head_conv
        self.first_level = int(np.log2(down_ratio))  # con 2 → first_level=1
        ...
        # Swin multi-escala (defaults plug-and-play)
        ...
        # Decoder DLAUp usa desde first_level
        scales = [2 ** i for i in range(len(channels[self.first_level:]))]
        self.dla_up = dla_modules.DLAUp(channels[self.first_level:], scales=scales)

        # Bottleneck conv en la última escala (opcional, mantiene C)
    ...
    def forward(self, input: torch.Tensor):
        # Features multi-escala del DLA
        encode = self.base_0(input)  # lista de (B,C,H,W)

        # Bottleneck en el último nivel antes de Swin
        encode[-1] = self.bottleneck_conv(encode[-1])

        # Refinar con Swin (incluye residual por escala)
        enhanced_encode = self.multiscale_swin(encode)

        # Decoder desde first_level (p.ej., 1 si down_ratio=2)
        decode_hm = self.dla_up(enhanced_encode[self.first_level:])
        heatmap = self.loc_head(decode_hm)
        return heatmap

    def freeze(self, layers: list) -> None:
        """Congela capas listadas por nombre de atributo."""
        for layer in layers:
