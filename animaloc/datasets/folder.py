...
class FolderDataset(CSVDataset):
    ...
    def __init__(
        ...
        super(FolderDataset, self).__init__(
            csv_file, root_dir, albu_transforms, end_transforms
        )

        # 2026-04-27: added .tif/.tiff extensions for full-frame deployment
        # inference on georeferenced GeoTIFFs (CAH 2022 / PCH 2017 source rasters).
        self.folder_images = [
            i
        ...
        # 2026-04-27: bug fix — when self.data['images'] holds full paths but
        # self.folder_images holds basenames, setdiff1d treats them as disjoint
        # and re-adds every basename, doubling the dataset and scrambling labels
        # (49/50 mismatched on the CAH inference run). Normalize CSV image paths
        # to basenames so setdiff and groupby compare like to like.
        self.data["images"] = self.data["images"].apply(
            lambda v: os.path.basename(v) if isinstance(v, str) else v
        )

        self.anno_keys = self.data.columns
        self.data["from_folder"] = 0

        folder_only_images = numpy.setdiff1d(
            self.folder_images, self.data["images"].unique().tolist()
        )
        folder_df = pandas.DataFrame(data=dict(images=folder_only_images))
        folder_df["from_folder"] = 1

        self.data = pandas.concat(
            [self.data, folder_df], ignore_index=True
        ).convert_dtypes()

        self._ordered_img_names = group_by_image(self.data)["images"].values.tolist()
        # 2026-04-27: bug fix — _img_names was set to self.folder_images (OS
        # listdir order) but iteration uses _ordered_img_names (alphabetical).
        # That mismatch scrambled detection-CSV labels via
        # evaluator.detections's `det['images'] = img_names[det['images']]`.
        # Aligning _img_names with the iteration order fixes the labeling.
        self._img_names = self._ordered_img_names
