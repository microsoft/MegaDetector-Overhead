...
from .lmds import HerdNetLMDS, HerdNet_Detection_Branch_LMDS, HerdNet_Detection_Count_Branch_LMDS
...
class Evaluator:
    ...
    def evaluate(self, returns: str = 'recall', wandb_flag: bool = False, viz: bool = False,
        ...
        for i, (images, targets) in enumerate(logger.log_every(self.dataloader, self.print_freq, self.header)):
            ...
            if viz and self.vizual_fn is not None:
                if i % self.print_freq == 0 or i == len(self.dataloader) - 1:
                    fig = self._vizual(image = images, target = targets, output = output_heatmap)
        ...
        # Write final aggregated metrics to log file
        if logger.logger is not None:
            summary_parts = [
                f'recall: {self.metrics.recall():.4f}',
                f'precision: {self.metrics.precision():.4f}',
                f'f1_score: {self.metrics.fbeta_score():.4f}',
            ...
            logger.logger.info(f'{self.header} SUMMARY | ' + ' | '.join(summary_parts))
...
@EVALUATORS.register()
class HerdNet_Detection_Branch_Evaluator(Evaluator):
    ...
    def post_stitcher(self, output: torch.Tensor) -> Any: # This output is a heatmap
        return output

    def prepare_feeding(self, targets: Dict[str, torch.Tensor], output: List[torch.Tensor]) -> dict:
        gt_coords = [p[::-1] for p in targets['points'].squeeze(0).tolist()]
        gt_labels = targets['labels'].squeeze(0).tolist()
        # convert all gt_labels to 1
        gt_labels = [1 for _ in gt_labels]
        ...
        lmds = HerdNet_Detection_Branch_LMDS(up=up, **self.lmds_kwargs)
        counts, locs, labels, dscores = lmds(output)
...
@EVALUATORS.register()
class HerdNet_Detection_Count_Branch_Evaluator(HerdNet_Detection_Branch_Evaluator):
    def __init__(self, model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, metrics: Metrics,
        lmds_kwargs: dict = {'kernel_size': (3,3)}, device_name: str = 'cuda', print_freq: int = 10,
        stitcher: Optional[Stitcher] = None, vizual_fn: Optional[Callable] = None, work_dir: Optional[str] = None,
        header: Optional[str] = None
        ) -> None:
        super().__init__(model, dataloader, metrics, lmds_kwargs=lmds_kwargs, device_name=device_name,
            print_freq=print_freq, stitcher=stitcher, vizual_fn=vizual_fn, work_dir=work_dir, header=header)

    def prepare_feeding(self, targets: Dict[str, torch.Tensor], output: tuple) -> dict:
        gt_coords = [p[::-1] for p in targets['points'].squeeze(0).tolist()]
        gt_labels = targets['labels'].squeeze(0).tolist()
        # convert all gt_labels to 1
        gt_labels = [1 for _ in gt_labels]
        ...
        lmds = HerdNet_Detection_Count_Branch_LMDS(up=up, **self.lmds_kwargs)
        counts, locs, labels, dscores = lmds(output)
