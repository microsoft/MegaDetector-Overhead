...
__all__ = ['LMDS', 'HerdNetLMDS', 'HerdNet_Detection_Branch_LMDS', 'HerdNet_Detection_Count_Branch_LMDS']
...
class HerdNet_Detection_Branch_LMDS(LMDS):
...
class HerdNet_Detection_Count_Branch_LMDS(LMDS):
    ...
    def __call__(self, outputs: List[torch.Tensor]) -> Tuple[list, list, list, list, list]:
        '''
        Args:
            outmaps (torch.Tensor): outputs of HerdNet localization and count branches, i.e. 1 tensor for heatmaps and another for counts:
                - heatmap: [B,1,H,W], 
                - counts: [B,1]
        ...
        Returns:
            Tuple[list,list,list,list,list]
                counts, locations, labels, class scores and detection scores per batch
        '''
        heatmap, counts = outputs
        counts = torch.round(counts).long()
