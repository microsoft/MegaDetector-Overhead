...
parser.add_argument('-skip_model_inference', action='store_true',
    help='if set, skips the model inference step (for debugging purposes).')
...
def main():
    ...
    if not args.skip_model_inference:
        ...
        num_classes = checkpoint['model_state_dict']['model.cls_head.2.weight'].shape[0]
        assert num_classes == 7, 'This code is currently hardcoded for 7 classes. Please update it accordingly.'
        classes = {1:'class_1', 2:'class_2', 3:'class_3', 4:'class_4', 5:'class_5', 6:'class_6'}
        ...
        img_names = [i for i in os.listdir(args.root) 
                if any([i.endswith(ext) for ext in possible_extensions])]
        n = len(img_names)
        ...
        metrics = PointsMetrics(20, num_classes = num_classes)
        ...
        date = '20250318'
        detections = pandas.read_csv(os.path.join(args.root, f'{date}_HerdNet_results', f'{date}_detections.csv'))
