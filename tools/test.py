...
import PIL
PIL.Image.MAX_IMAGE_PIXELS = None  # Disable the limit on image size
...
def _build_model(cfg: DictConfig) -> torch.nn.Module:
    ...
    # Single-class heatmap models (OWL family): num_classes is implicit (=1)
    # and must not be set in the config. All other models require num_classes.
    if name in ('OWLC', 'OWLT', 'OWLD_S', 'OWLD_B', 'OWLD_L', 'OWLD_H'):
        assert num_classes is None, \
            f'\'num_classes\' should not be set for {name} model, got {num_classes}'
    else:
...
def main(cfg: DictConfig) -> None:
    ...
    if anno_type == 'point':
        metrics = PointsMetrics(radius = cfg.evaluator.threshold, num_classes = num_classes if num_classes is not None else 2) # 2 is the default value
    elif anno_type == 'bbox':
        metrics = BoxesMetrics(iou = cfg.evaluator.threshold, num_classes = num_classes if num_classes is not None else 2) # 2 is the default value
    else:
